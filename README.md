...
- 🔘 **物理按键控制**：支持单击（开关）、双击（亮度+10%）、长按（颜色循环）三种操作模式，采用双阈值消抖算法确保可靠性；
...
|  | B | GPIO18 | 蓝色 PWM |
|  | GND | GND | 地 |
| **物理按键1 卧室灯** | 一端 | GPIO40 | 卧室灯控制按键（低电平有效） |
|  | 另一端 | GND | 地 |
| **物理按键2 客厅灯** | 一端 | GPIO41 | 客厅灯控制按键（低电平有效） |
|  | 另一端 | GND | 地 |

> **注意**：所有模块的 GND 必须共地；INMP441 和 NS4168 的电源建议加 10μF + 0.1μF 去耦电容；物理按键使用 ESP32 内部上拉电阻（~40KΩ）。
...
| Key_Control 任务 | 12 | 4096 | 物理按键控制灯任务，消费KeyManager事件并控制LED（单击开关/双击调亮度/长按换颜色） | 高优先级保证实时响应 |
...
## 版本说明
V1：初始版本，支持语音控制卧室灯/客厅灯。
V2：增加米家 App 控制功能，可以直接叫手机的小爱同学了。
V2.1：增加物理按键